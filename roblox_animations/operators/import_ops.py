...
class OBJECT_OT_ImportModel(bpy.types.Operator, ImportHelper):
    ...
    filepath: bpy.props.StringProperty(
        name="File Path", maxlen=1024, default="")
    ...
    def execute(self, context):
        ...
        imported_objs = {obj for obj in bpy.data.objects if obj.name not in objnames_before_import}
        ...
        # Now, delete the meta objects
        for obj in bpy.data.objects:
            obj.select_set(False)
        for obj in meta_objs_to_delete:
            obj.select_set(True)
        if meta_objs_to_delete: # Only delete if there's something to delete
        ...
        try:
            for i in range(1, len(partial) + 1):
                if i in partial:  # Check if the key exists
                    ...
                    self.report(
                        {'ERROR'}, f"Missing metadata part {i}. The rig file may be corrupted.")
                    return {'CANCELLED'}
            ...
            if 'parts' in meta_loaded and 'rigName' in meta_loaded:
                ...
                autoname_parts(
                    meta_loaded['parts'], meta_loaded['rigName'], parts_collection.objects)
            else:
                self.report(
                    {'WARNING'}, "Missing 'parts' or 'rigName' in rig metadata")
            
            return {'FINISHED'}
        except KeyError as e:
            self.report(
                {'ERROR'}, f"KeyError: {str(e)} - The rig file may be corrupted or incompatible.")
            return {'CANCELLED'}
...
class OBJECT_OT_ImportFbxAnimation(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        armatures_imported = [
            x for x in bpy.data.objects if x.type == 'ARMATURE' and x.name in objnames_imported]
        if len(armatures_imported) != 1:
            self.report({'ERROR'}, 'Imported file contains {:d} armatures, expected 1.'.format(
        ...
        if len(err_mappings) > 0:
            self.report({'ERROR'}, 'Cannot map rig, the following bones are missing from the source rig: {}.'.format(
                ', '.join(err_mappings)))
            clear_imported()
            return {'FINISHED'}
        ...
        fcurves = get_action_fcurves(ao_imp.animation_data.action) if ao_imp.animation_data and ao_imp.animation_data.action else []
        if not ao_imp.animation_data or not ao_imp.animation_data.action or not fcurves:
            self.report(
                {'ERROR'}, 'Imported armature contains no animation data.')
            clear_imported()
            return {'FINISHED'}
        ...
        try:
            ...
            self.report(
                {'ERROR'}, f"No object named '{armature_name}' found. Please ensure the correct rig is selected.")
            return {'FINISHED'}

        if armature.animation_data is None:
            self.report(
                {'ERROR'}, f"The object '{armature_name}' has no animation data. Please ensure the correct rig is selected.")
            return {'FINISHED'}
