    ...
    def do_GET(self):
        ...
        if self.path.startswith("/animation_status"):
            ...
            if not (obj and obj.type == "ARMATURE"):
                self.send_detailed_error(
                    404, "Not Found", "Armature not found")
                return
            ...
            try:
                ...
                print(
                    f"Blender Addon: Queued task {('list_armatures', task_id)}")
                ...
                if not success:
                    print(
                        f"Blender Addon: Task {task_id} failed. Sending 500 error.")
            ...
            armature_name = urllib.parse.unquote(armature_name_encoded)
            try:
                from .requests import pending_requests, pending_responses
                
                task_id = str(time.time())
                pending_requests.append(
                    ('get_bone_rest', task_id, armature_name))
            ...
            try:
                ...
                task_id = str(time.time())
                pending_requests.append(
                    ('export_animation', task_id, armature_name))
    ...
    def do_POST(self):
        ...
        if self.path.startswith('/import_animation'):
            ...
            try:
                ...
                task_id = str(time.time())
                pending_requests.append(('import', task_id, animation_data, target_armature))
